...
CUDA memory from ingest through post-processing to final encode. Here, 
frames are decoded with NVDEC, processed on a user-supplied CUDA stream 
and encoded with NVENC after a GPU-side color conversion to NV12. Decode, 
compute, and encode run on separate CUDA streams and are synchronised with 
CUDA events so that the specialised hardware engines can overlap as much 
work as possible.
"""
